...
public class Heater475 {
    ...
    public static void main(String args[]){
        ...
        for(;i<houses.length;i++){
            if(heaters[0]==houses[i])break;
            startWidth++;
        }
        i++;
        int maxwidth = 0;
        for(int j =1;j<heaters.length && i<houses.length;j++){
        ...
        int endWidth =houses.length-i;

        int ans = (endWidth > startWidth ? endWidth :startWidth);

        ans = ans > (maxwidth+1)/2 ? ans : (maxwidth+1)/2;
